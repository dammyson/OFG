...
import com.ofidy.ofidyshoppingbrowser.netcipher.proxy.OrbotHelper;
import com.ofidy.ofidyshoppingbrowser.netcipher.webkit.WebkitProxy;
...
public class ProxyUtils {
    ...
    private void initializeProxy(@NonNull Activity activity) {
        ...
        try {
            WebkitProxy.setProxy(Ofidy.class.getName(), activity.getApplicationContext(), null, host, port);
        } catch (Exception e) {
    ...
    public void updateProxySettings(@NonNull Activity activity) {
        if (mPreferences.getUseProxy()) {
            ...
            try {
                WebkitProxy.resetProxy(Ofidy.class.getName(), activity.getApplicationContext());
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static int setProxyChoice(int choice, @NonNull Activity activity) {
        switch (choice) {
            ...
            case Constants.PROXY_I2P:
                I2PAndroidHelper ih = new I2PAndroidHelper(Ofidy.get(activity));
