...
import com.ofidy.ofidyshoppingbrowser.Materials.Events.BookmarkEvents;
import com.ofidy.ofidyshoppingbrowser.Materials.Events.BrowserEvents;
...
import com.ofidy.ofidyshoppingbrowser.Materials.utils.DrawableUtils;
...
import com.ofidy.ofidyshoppingbrowser.Materials.utils.UrlUtils;
import com.ofidy.ofidyshoppingbrowser.Materials.utils.Utils;
import com.ofidy.ofidyshoppingbrowser.Materials.utils.WebUtils;
...
import com.ofidy.ofidyshoppingbrowser.bonsai.Schedulers;
import com.ofidy.ofidyshoppingbrowser.ofidyApp.ui.*;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.constant.BookmarkPage;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.constant.Constants;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.constant.HistoryPage;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.controller.UIController;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.database.BookmarkManager;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.database.HistoryDatabase;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.database.HistoryItem;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.interpolator.BezierDecelerateInterpolator;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.receiver.NetworkReceiver;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.search.SuggestionsAdapter;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.browser.BookmarksView;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.browser.BrowserPresenter;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.browser.BrowserView;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.browser.TabsView;
...
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.view.AnimatedProgressBar;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.view.Handlers;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.view.LightningView;
import com.ofidy.ofidyshoppingbrowser.ofidyExtra.ui.view.SearchView;
...
public abstract class BrowserActivity extends ThemableBrowserActivity implements BrowserView, UIController, OnClickListener, OnLongClickListener {
    ...
    public void onAddToCartClicked(){
        ...
        if(!TextUtils.isEmpty(currentTab.getUrl()) && currentTab.getUrl().contains("ofidy.com")){
            final AlertDialog alertDialog = new AlertDialog.Builder(this)
                    .setTitle("Can't add to cart")
                    .setMessage("This shopping browser is for shopping from other website")
     ...
     public void goToApp(){
          ...
          Intent intent = new Intent(this, com.ofidy.ofidyshoppingbrowser.ofidyApp.ui.MainActivity.class);
...
//        mBrowserFrame.setLayoutTransition(new LayoutTransition());
    ...
    protected void onPause() {
        ...
        try {
            Ofidy.get(this).unregisterReceiver(mNetworkReceiver);
    ...
    protected void onResume() {
        ...
        Ofidy.get(this).registerReceiver(mNetworkReceiver, filter);
    ...
    public void updateUrl(@Nullable String url, boolean shortUrl) {
        ...
        final LightningView currentTab = mTabsManager.getCurrentTab();
        if(!TextUtils.isEmpty(currentTab.getUrl()) && currentTab.getUrl().contains("ofidy.com")){
            //System.out.println("........................................loginsupdateUrl" + currentTab.getUrl());
        }
    ...
    public void updateProgress(int n) {
        setIsLoading(n < 100);
        if(n >= 80 && n <= 83){
            System.out.println("........................................loginsprogress");
        }
    ...
    void addItemToHistory(@Nullable final String title, @NonNull final String url) {
        ...
        Ofidy.getIOThread().execute(new Runnable() {
            @Override
            public void run() {
                try {
...
//        }
    ...
    class SendParams extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(final String... params) {
            ...
            try {
                ...
                RequestBody formBody = new FormBody.Builder()
                        .add("user_id", UserPrefs.getInstance(BrowserActivity.this).getString(UserPrefs.Key.ID))
                        .add("session_id", UserPrefs.getInstance(BrowserActivity.this).getString(UserPrefs.Key.SID))
                        .add("currency",  Ofidy.getInstance().getCurrency())
                        .add("acctype", String.valueOf(size))
                        .add("guestlogin", UserPrefs.getInstance(BrowserActivity.this).getString(UserPrefs.Key.EMAIL).
                                equals("guest")? "1" : "0")
                        .build();
                Request request = new Request.Builder()
